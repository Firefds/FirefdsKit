...
public class XSecContactsResources {
	...
	public static void doHook(XSharedPreferences prefs, InitPackageResourcesParam resparam,
		...
		XSecContactsResources.resparam = resparam;
		XSecContactsResources.moduleResources = moduleResources;

		if (prefs.getBoolean("enableCallButtonLogs", false) && prefs.getBoolean("fixCallButtonLogs", false)) {
			try {
	...
	private static void replaceCallLogButtonDrawables() {
		...
		resparam.res.setReplacement(Packages.CONTACTS, "drawable", "log_list_call_btn", moduleResources
				.fwd(moduleResources.getIdentifier("favorites_icon_call", "drawable", Packages.FIREFDSKIT)));
